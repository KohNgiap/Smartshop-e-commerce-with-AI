...
def api_chat(request):
    ...
    # ---- Intent detection (supports under/above + high price queries) ----
    import re

    amount_match = re.search(r"\$?\s*(\d+)", ql)
    ...
    wants_high = any(w in ql for w in ["most expensive", "higher price", "high price", "expensive", "premium", "costly"])
    ...
    noise = {
        ...
        "most", "expensive", "higher", "high", "price", "prices",
        "below", "under", "less", "than", "above", "over", "more",
        "products", "product", "dollar", "dollars"
    }
    ...
    # Keyword filter only if keywords exist
    if keywords:
        kw = Q()
        for w in keywords[:6]:
            kw |= (
    ...
    # ---- IMPORTANT FIX: High-price intent without keywords must still return results ----
    if wants_high and amount is None:
        qs = qs.order_by("-price", "-id")  # top expensive products
        title = "Here are some higher-priced products from our catalog:"
    elif amount is not None and (wants_over or wants_high):
        qs = qs.order_by("-price", "-id")
        title = f"Here are some products priced above ${amount}:"
    elif amount is not None and wants_under:
        qs = qs.order_by("price", "id")
        title = f"Here are some products under ${amount}:"
    else:
        qs = qs.order_by("-id")
    ...
    # Deterministic reply (always contains real product names)
    lines = [title]
    for p in products:
        lines.append(f"- {p.name} (${p.price}) — {p.category} (View: /products/{p.id}/)")
    deterministic = "\n".join(lines)

    # Optional AI polish (mocked in tests; safe if returns empty)
...
Reply in short bullet points with prices.
"""
    ai_reply = (generate_text(prompt) or "").strip()

    return Response({"reply": ai_reply if ai_reply else deterministic})
